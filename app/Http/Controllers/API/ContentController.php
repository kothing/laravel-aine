    ...
    public function getContent($uuid, $slug, Request $request){
        ...
        if($request->has('whereRelation')){
            ...
            if(!is_array($whereRelation)) {
                return response(['error' => 'Incorrect whereRelation statement. See documentation: #where-through-relation'], 422);
            }
        ...
        if($request->has('offset') && !$request->has('limit')){
            return response(['error' => 'Incorrect offset statement. Offset must be used with limit. Documentation: #limit'], 422);
        }
