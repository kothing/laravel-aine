<template>
    <div class="admin__media-library">
        <h4 class="mb-2 py-2 font-bold text-lg">
            ...
            <ui-button v-if="!showUploader" color="indigo-500" class="float-right" @click.native="showUploader = true">
                <i class="fas fa-upload text-white mr-3"></i> Upload Files
            </ui-button>

            <ui-button v-if="!showUploader && selectedFiles.length !== 0 && !disableDelete" color="red-500" class="float-right mr-2" @click.native="deleteSelected()">
                <i class="fas fa-trash text-white mr-3"></i> Delete ({{ this.selectedFiles.length }})
            </ui-button>

            <ui-button v-if="!showUploader && hasInsertListener" color="green-500" class="float-right mr-2" @click.native="insertFiles()">
                <i class="fas fa-plus text-white mr-3"></i> Insert Files ({{ this.selectedFiles.length }})
            </ui-button>

            <ui-button v-if="showUploader" color="green-500" class="float-right" @click.native="hideUploader">
                <i class="fas fa-chevron-left text-white mr-3"></i> File List
            </ui-button>

            <ui-button color="indigo-500" class="float-right mr-2" v-if="showUploader && files.length != 0 && (!$refs.upload || !$refs.upload.active)" @click.native="$refs.upload.active = true">
                Start Upload
            </ui-button>
        </h4>
                    ...
                    <div v-if="selectedFile !== null">
                        ...
                        <div>
                            ...
                            <div class="float-left text-sm">
                                ...
                                <div>
                                    {{ selectedFile.created_at | date("D MMM YYYY") }}
                                </div>
            ...
            <div class="mt-4">
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                    <div class="space-y-1 text-center">
                        ...
                        <p class="text-xs text-gray-500" v-if="upload_max_filesize !== null">Up to {{ upload_max_filesize | prettyBytes(1024) }}</p>
                        <p class="text-sm text-gray-500 pt-5 w-full text-center" v-if="typeof project.id !== 'undefined' && checkRole(['admin' + project.id]) && project.s3">
                            ...
                            <strong>{{ project.disk == "s3" ? "AWS S3" : "Local" }}</strong
                    ...
                    <div v-for="file in files" :key="file.id" class="relative">
                        ...
                        <div class="w-full mt-1">
                            ...
                            <div class="text-sm float-right">
                                <i v-if="!file.success" class="fa fa-times-circle text-red-400 cursor-pointer hover:text-red-600" @click.prevent="$refs.upload.remove(file)"></i>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        getMedia(page) {
            ...
            axios
                .get("/admin/media/get-files/" + this.project.id + "?page=" + page, {
                    ...
                    if (result.value) {
                        axios.delete("/admin/media/delete/" + this.$route.params.project_id + "/" + file.id, { params: { project_id: this.project.id } }).then((response) => {
                            ...
                            for (let i = 0; i < this.selectedFiles.length; i++) {
                                ...
                                if (file.id === element) this.$delete(this.selectedFiles, i);
                            }
