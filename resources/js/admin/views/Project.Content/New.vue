<template>
    <div class="admin__project-content-new relative h-full flex flex-col">
        ...
        <div class="flex flex-1 overflow-y-auto">
            ...
            <div class="w-9/12 p-4 overflow-x-auto">
                <div class="mb-2 py-2 font-bold text-lg flex justify-between">
                    ...
                    <div class="flex">
                        <ui-button :color="'indigo-500'" class="rounded-r-none" @click="saveNew(false)">{{ __('Save') }}</ui-button>

                        <div class="flex border-indigo-600 border border-l-1 border-t-0 border-b-0 border-r-0">
                            <ui-dropdown align="right">
                                <template #trigger>
                                    <button
                                        class="bg-indigo-500 hover:bg-indigo-600 items-center px-2 py-2 border border-transparent rounded-md text-sm text-white focus:outline-none transition ease-in-out duration-150 rounded-l-none"
                                    >
                                ...
                                <template #content>
                                    <ui-button type="button" color="white" hover="gray-200" class="w-full h-full an__rounded-b-none" @click="saveNew(false, 'close')">
                                        <div class="text-gray-600 text-sm text-left">{{ __('Save and close') }}</div>
                                    </ui-button>
                                    <ui-button type="button" color="white" hover="gray-200" class="w-full h-full an__rounded-t-none" @click="saveNew(false, 'new')">
                        ...
                        <ui-button :color="'green-500'" class="rounded-r-none ml-2" @click="saveNew(true)">{{ __('Save and Publish') }}</ui-button>
                        <div class="flex border-green-600 border border-l-1 border-t-0 border-b-0 border-r-0">
                            <ui-dropdown align="right" width="60">
                                ...
                                <template #content>
                                    <ui-button type="button" color="white" hover="gray-200" class="w-full h-full an__rounded-b-none" @click="saveNew(true, 'close')">
                                        <div class="text-gray-600 text-sm text-left">{{ __('Save, publish and close') }}</div>
                                    </ui-button>
                                    <ui-button type="button" color="white" hover="gray-200" class="w-full h-full an__rounded-t-none" @click="saveNew(true, 'new')">
                ...
                <div
                    class="grid grid-cols-6 space-x-4 h-full overflow-hidden"
                >
                                            ...
                                            <div class="absolute inset-0 rounded-md bg-black bg-opacity-70 items-center text-center flex opacity-0 hover:opacity-100 z-10">
                                                <div class="w-full">
                                                    ...
                                                    <div class="text-sm w-full">
                                                        ...
                                                        <i
                                                            class="fa fa-minus-circle text-gray-100 cursor-pointer hover:text-gray-200 text-lg ml-2"
                                                            @click="removeFile(file, field.name)"
                                                        ></i>
                                                    </div>
                                                </div>
                                            </div>
                                                    ...
                                                    <tbody>
                                                        <tr v-for="item in relationRecords[field.name]" :key="item.id">
                                                            <td class="pl-2 py-4 text-sm text-center w-px">
                                                                <a
                                                                    class="text-red-500 px-3 py-1 rounded-md hover:bg-gray-100 cursor-pointer bg-gray-50"
                                                                    @click="removeRelation(item, field.name)"
                                                                >
                                                            ...
                                                            <td class="pl-2 py-4 text-sm text-center w-px">
                                                                <span v-if="item.published_at !== null" class="text-gray-500 text-sm rounded-md bg-green-200 px-3 py-1">
                                                                    {{ __('published') }}
                                                                </span>
...
export default {
    ...
    data() {
        return {
            ...
            // page's own getNew() refreshes the data.
    ...
    methods: {
        getNew() {
            axios.get("content/new/" + this.$route.params.project_id + "/" + this.$route.params.col_id).then((response) => {
        ...
        saveNew(published, after = null) {
            ...
            axios.post("content/store/" + this.$route.params.project_id + "/" + this.$route.params.col_id, this.newData).then(
                (response) => {
        ...
        async insertFiles(files) {
            await axios
                .post("content/get-selected-files/" + this.$route.params.project_id, {
                    data: files,
                })
                .then((response) => {
        ...
        async addSelectedRelation(data) {
            ...
            await axios
                .post("content/get-selected-records/" + this.$route.params.project_id, {
                    data: data,
                })
                .then((response) => {
        ...
        removeRelation(item, field_name) {
            for (let i = this.newData.data[field_name].length - 1; i >= 0; i--) {
                if (this.newData.data[field_name][i] == item.id) {
                    this.newData.data[field_name].splice(i, 1);
                }
            }
            for (let i = this.relationRecords[field_name].length - 1; i >= 0; i--) {
                if (this.relationRecords[field_name][i].id == item.id) {
                    this.relationRecords[field_name].splice(i, 1);
                }
            }
        },
